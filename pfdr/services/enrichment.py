...
class AbstractEnrichmentService:
    ...
    async def enrich_abstract(self, paper: Paper) -> Optional[str]:
        ...
        if paper.doi or paper.title:
            ...
            if abstract:
                logger.info(f"Found abstract via Semantic Scholar for {paper.identifier}")
                return abstract
                ...
                async with self.session.get(search_url, params=params) as response:
                    if response.status == 200:
                        ...
                        if papers:
                            ...
                            best_match = self._find_best_title_match(paper.title, papers)
                            if best_match:
...
Return only technical keywords separated by commas, no explanations."""
        ...
        try:
            ...
            logger.error(f"Failed to extract keywords for paper {paper.identifier}: {e}")
            return []
...
class PaperClusteringService:
    ...
    async def cluster_papers(self, papers: List[Paper], existing_categories: List[str] = None) -> Dict[str, str]:
    ...
    async def _determine_categories(self, papers: List[Paper], existing_categories: List[str] = None) -> List[str]:
...
Return only the category names, one per line, no explanations. Use clear, concise names."""
    ...
    async def _assign_paper_to_category(self, paper: Paper, categories: List[str]) -> str:
...
class PaperEnrichmentService:
    ...
    async def enrich_papers_batch(
        ...
        papers_needing_categories = [
            paper for paper, fields in papers_to_enrich 
        ...
        if papers_needing_categories:
            ...
            category_assignments = await self.clustering_service.cluster_papers(papers_needing_categories, existing_categories)
            for result in results:
                if result.success and result.paper in papers_needing_categories:
                    result.category = category_assignments.get(result.paper.identifier, "other")
